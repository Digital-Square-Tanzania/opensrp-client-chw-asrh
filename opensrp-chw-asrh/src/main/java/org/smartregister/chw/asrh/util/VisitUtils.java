...
import org.smartregister.chw.asrh.repository.VisitDetailsRepository;
...
public class VisitUtils {
    ...
    public static void processVisits(VisitRepository visitRepository, VisitDetailsRepository visitDetailsRepository, Context context) throws Exception {
        ...
        for (Visit v : visits) {
            ...
            if (v.getVisitType().equalsIgnoreCase(Constants.EVENT_TYPE.ASRH_FOLLOW_UP_VISIT)) {
                try {
                    visitList.add(v);
                } catch (Exception e) {
                    Timber.e(e);
    ...
    public static List<Visit> getVisits(String memberID, String... eventTypes) {
        ...
        List<Visit> visits = (eventTypes != null && eventTypes.length > 0) ? getVisitsOnly(memberID, eventTypes[0]) : getVisitsOnly(memberID, Constants.EVENT_TYPE.ASRH_FOLLOW_UP_VISIT);
